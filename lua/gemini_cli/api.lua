...
---Toggle gemini terminal in YOLO mode
---@param opts? table Optional configuration override
function M.toggle_terminal_yolo(opts)
  opts = opts or {}
  opts.yolo = true
  terminal.toggle(opts)
end
...
function M.check_terminal_response(opts)
  ...
  for _, line in ipairs(lines) do
    ...
    if
      lower_line:match("error")
      or lower_line:match("not found")
      or lower_line:match("invalid")
      or lower_line:match("cannot")
      or lower_line:match("failed")
    then
      has_error = true
    end
    if
      lower_line:match("loaded")
      or lower_line:match("reading")
      or lower_line:match("analyzing")
      or lower_line:match("file")
      or lower_line:match("added")
    then
      has_success = true
    end
    -- 检查是否有文件内容的迹象
    if line:match("```") or line:match("def ") or line:match("function") or line:match("class ") or line:match("#") then
  ...
  elseif #recent_output == 0 or #lines <= 1 then
    vim.notify(
      "⚠️  @filename command not supported. Try: :lua require('gemini_cli').api.add_current_file_content()",
      vim.log.levels.WARN
    )
  else
